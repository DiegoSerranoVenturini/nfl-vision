...
class CnnBlock(nn.Module):
    ...
    def output_size(self, input_size=256):
        ...
        if self.pool_kernel_size:
            output_block_w = np.floor((output_cnn_w - (self.pool_kernel_size-1)-1) / self.pool_kernel_size - 1)
            output_block_h = np.floor((output_cnn_h - (self.pool_kernel_size-1)-1) / self.pool_kernel_size - 1)
        else:
            output_block_w = output_cnn_w
            output_block_h = output_cnn_h
...
class CNN(nn.Module):
    ...
    def __init__(self, img_size, n_blocks, n_classes, n_channels_in=3, channel_increase_rate=2, kernel_size=5, padding=2, stride=1,
        ...
        self.conv_blocks = nn.ModuleList(
            ...
            [CnnBlock(n_channels_in*(channel_increase_rate*i), n_channels_in*(i+1*channel_increase_rate), kernel_size, padding, stride, dilation, pool_kernel_size) for i in range(1, n_blocks)]
        )
        ...
        self.linear_out = nn.Linear(img_size_output[0]*img_size_output[1]*channel_increase_rate*n_blocks, n_classes)
        self.logsoftmax = F.log_softmax(n_classes)
